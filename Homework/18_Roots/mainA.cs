...
public class main{
	public static int Main(){
		...
		matrix xs = new matrix("-0.5 0 0.5");
                ...
                for(int i=0;i<3;i++){
                        vector x = xs[i];
                        (vector xmin,vector fmin,int steps) = root.newton(f1,x);
		...
		Func<vector,vector> f2 = delegate(vector v){vector w = new vector(2); f_eval++;
                ...
                for(int i=0;i<3;i++){
                        vector x = xs2[i];
                        (vector xmin,vector fmin,int steps) = root.newton(f2,x);
		...
		matrix xs3 = new matrix("0.5 1.2 2");
                ...
                for(int i=0;i<3;i++){
                        vector x = xs3[i];
                        (vector xmin,vector fmin,int steps) = root.newton(f3,x);
		...
		for(int i=0;i<3;i++){
			vector x0 = x0s_R[i];
                        (vector xmin,vector dfmin,int steps) = root.newton(dRf,x0);
			double fmin = Rf(xmin);
		...
		WriteLine("f(vmin) = 0, df(vmin) = 0");
                ...
                for(int i=0;i<4;i++){
                        vector x0 = x0s_H[i];
                        (vector xmin,vector dfmin,int steps) = root.newton(dHf,x0);
                        double fmin = Hf(xmin);
                        WriteLine($"Numerical results, v0 = ({x0[0]}, {x0[1]}):");
                        WriteLine($"vmin = ({xmin[0]}, {xmin[1]}). f(vmin) = {fmin}. df(vmin) = ({dfmin[0]}, {dfmin[1]})");
                        WriteLine($"Steps taken: {steps}. Function evals: {f_eval}");
