...
public class main{
	public static int Main(){
		...
		Func<vector,vector> f1 = delegate(vector v){vector w = new vector(1); f_eval++;
		       					w[0] = v[0]*Exp(-v[0]*v[0]); return w;};
		matrix xs = new matrix("-0.5 0 0.5");
                WriteLine("f = x*e(-x^2). v0 = 0");
                WriteLine("");
                for(int i=0;i<3;i++){
                        vector x = xs[i];
                        (vector xmin,vector fmin,int steps) = root.newton_quad_int(f1,x);
		...
		Func<vector,vector> f2 = delegate(vector v){vector w = new vector(2); f_eval++;
							w[0] = 1+v[1]*v[0]; w[1] = 1-v[0]*v[0]; return w;};
                matrix xs2 = new matrix("-0.5 0.001 0.001;0 -0.001 0.5");
                WriteLine("(x,y) -> (1+xy, 1-x^2). v0 = (1,-1) & (-1,1)");
                WriteLine("");
                for(int i=0;i<3;i++){
                        vector x = xs2[i];
                        (vector xmin,vector fmin,int steps) = root.newton_quad_int(f2,x);
		...
		Func<vector,vector> f3 = delegate(vector v){vector w = new vector(2); f_eval++;
		       					w[0] = Pow(v[0]-1,2); w[1] = Log(v[0]); return w;};
		matrix xs3 = new matrix("0.5 1.2 2");
                WriteLine("f = ((x-1)^2, ln(x)). x0 = 1");
                WriteLine("");
                for(int i=0;i<3;i++){
                        vector x = xs3[i];
                        (vector xmin,vector fmin,int steps) = root.newton_quad_int(f3,x);
		...
		Func<vector,double> Rf = delegate(vector v){f_eval++;return Pow(1-v[0],2)+100*Pow(v[1] - v[0]*v[0],2);};
		Func<vector,vector> dRf = delegate(vector v){vector w = new vector(2); f_eval++;
		...
		for(int i=0;i<3;i++){
			vector x0 = x0s_R[i];
                        (vector xmin,vector dfmin,int steps) = root.newton_quad_int(dRf,x0);
			double fmin = Rf(xmin);
		...
		//Himmelblau function part
                Func<vector,double> Hf = delegate(vector v){f_eval++;return Pow(v[0]*v[0]+v[1]-11,2)+Pow(v[0] + v[1]*v[1]-7,2);};
                Func<vector,vector> dHf = delegate(vector v){vector w = new vector(2); f_eval++;
		...
		WriteLine("f(vmin) = 0, df(vmin) = 0");
                ...
                for(int i=0;i<4;i++){
                        vector x0 = x0s_H[i];
                        (vector xmin,vector dfmin,int steps) = root.newton_quad_int(dHf,x0);
                        double fmin = Hf(xmin);
                        WriteLine($"Numerical results, v0 = ({x0[0]}, {x0[1]}):");
                        WriteLine($"vmin = ({xmin[0]}, {xmin[1]}). f(vmin) = {fmin}. df(vmin) = ({dfmin[0]}, {dfmin[1]})");
                        WriteLine($"Steps taken: {steps}. Function evals: {f_eval}");
