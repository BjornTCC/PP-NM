...
public class main{
	public static int Main(){
		...
		matrix vstart = new matrix("-3 -3 -2 3; -4 3 -3 -3"), vmin = new matrix(2,4);
		...
		WriteLine("");
                ...
                for(int i=0;i<4;i++){
			min.newton minimum = new min.newton(Rosen, vstart[i],acc:acc);
		...
		for(int i=0;i<4;i++){
                        min.newton minimum = new min.newton(Himmel, vstart[i],acc:acc);
